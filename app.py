...
st.markdown("Upload your cleaned churn datasets, train the model, and get intervention strategies for each user.")

# ─────────────────────────────────────────────
# HELPER: FEATURE ENGINEERING (from notebook)
# ─────────────────────────────────────────────
def engineer_features(df):
    ...
    # Total credit spent (use purchase_amount if available)
    ...
    # total_generations (placeholder — 0 if generations file not available)
...
# ─────────────────────────────────────────────
# HELPER: INTERVENTION STRATEGY (from notebook)
# ─────────────────────────────────────────────
...
# ══════════════════════════════════════════════
with tab1:
    st.header("Step 1 — Upload your 3 CSV files")

    col1, col2, col3 = st.columns(3)
    with col1:
        inv_file = st.file_uploader("🔴 Involuntary Churn CSV", type="csv", key="inv")
    with col2:
        nc_file = st.file_uploader("🟢 Not Churn CSV", type="csv", key="nc")
    with col3:
        vol_file = st.file_uploader("🟡 Voluntary Churn CSV", type="csv", key="vol")

    # Optional generations file
    gen_file = st.file_uploader("⚙️ Generations CSV (optional — for total_generations feature)", type="csv", key="gen")

    st.markdown("---")
    st.header("Step 2 — Train the Model")

    if st.button("🚀 Train Model", disabled=not (inv_file and nc_file and vol_file)):
        with st.spinner("Loading and engineering features..."):
            involuntary = pd.read_csv(inv_file)
            not_churn   = pd.read_csv(nc_file)
            voluntary   = pd.read_csv(vol_file)

            # Ensure churn_status column
            if 'churn_status' not in involuntary.columns:
                involuntary['churn_status'] = 'invol_churn'
            if 'churn_status' not in not_churn.columns:
                not_churn['churn_status'] = 'not_churned'
            if 'churn_status' not in voluntary.columns:
                voluntary['churn_status'] = 'vol_churn'

            df = pd.concat([involuntary, not_churn, voluntary], ignore_index=True)
            st.success(f"✅ Loaded {len(df):,} total rows")
            ...
            if gen_file is not None:
                ...
                if 'user_id' in generations.columns:
                    ...
                    st.success(f"✅ Merged generations data")
    ...
    elif not (inv_file and nc_file and vol_file):
        st.info("👆 Please upload all 3 CSV files to enable training.")
